...
export default function MatrixBackground() {
  ...
  // We are using a pseudo-element (`::before`) to apply the content and gradient.
  // This cannot be done with inline styles, so we inject a style tag.
  useEffect(() => {
    const styleTag = document.createElement('style');
    styleTag.id = 'matrix-styles';
    styleTag.innerHTML = `
      .matrix-container {
        position: absolute;
        top: 0;
    ...
    return () => {
      const existingStyleTag = document.getElementById('matrix-styles');
      if (existingStyleTag) {
  ...
  return (
    <div className="matrix-container" aria-hidden="true">
      {[...Array(numberOfPatterns)].map((_, patternIndex) => (
